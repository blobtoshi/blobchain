...
import type {
  ...
  Block, Tx, SubmitTxPayload, SubmitEntryPayload,
...
// -- ENTRY (single-shot, local client submission) -----------------------

// Phase 6: replaces the commit/reveal pair. Player sends one message with

// the full entry payload. We validate signature + PoW + simulator + window,

// then upsert into the entries table. Best-score-per-address-per-block

// applies: a second submission with a higher score replaces the prior row.
...
export function ingestEntry(d: DB, payload: SubmitEntryPayload): IngestEntryResult {
  ...
  if (existing && existing.score >= e.score) {
    ...
    // Lower or equal score - keep the existing entry. Not an error, just a no-op.

    return {

      ok: true, isNewBest: false,

      address: e.address, score: existing.score,

      block_height: e.block_height,

      block_seed: e.block_seed,

      signature: existing.signature ?? e.signature,
...
// -- ENTRY (peer gossip - re-validates with the same single-shot rules) ----

// Peers ship the full SubmitEntryPayload. We call the same validator a

// local client would so a malicious peer can't inject a forged entry.

export function ingestEntryFromPeer(d: DB, payload: SubmitEntryPayload): IngestEntryResult {

  return ingestEntry(d, payload);
...
//   2. height = tip     -> potential reorg vs. our just-sealed tip. Each node

//                         ran pickWinner on its own observed entry set, so

//                         when two nodes seal competing blocks we need to

//                         pick a winner *between* them. We compare

//                         `scoreWeight(winnerScore)` first - the same curve

//                         used inside pickWinner - and fall back to the old

//                         hash-based key only on exact weight equality.
...
export function ingestBlock(d: DB, block: Block): IngestBlockResult {
  ...
  if (isReplace) {
    ...
    //      the same address with the same score, or two blocks with no

    //      entries at all), fall back to the hash-based key. This preserves

    //      the deterministic, unforgeable ordering for the rare case where

    //      score-weight can't separate them.
